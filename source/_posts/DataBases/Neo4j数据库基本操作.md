...
```shell
docker pull neo4j
docker run -d --name neo4j -p 7474:7474 -p 7687:7687 -v docker/neo4j/conf:/var/lib/neo4j/conf  neo4
docker run -d --name neo4j -p 7474:7474 -p 7687:7687  -v /home/docker/neo4j/conf:/var/lib/neo4j/conf -v /home/docker/neo4j/import:/var/lib/neo4j/import  -v /home/docker/neo4j/data:/data -v /home/docker/neo4j/logs:/logs neo4j:4.4.10
```

​	-v /home/neo4j/data:/data   //把容器内的数据目录挂载到宿主机的对应目录下 -v /home/neo4j/logs:/logs   //挂载日志目录

### 创建节点

```neo4j
CREATE (ee:Person {name:'name'})
```
...
- `MATCH`	 ： 指定节点和关系的模式
- `(ee: Person) `: 是带标签的单节点模式， 它将匹配项分配给变量。 ee
- `WHERE`过滤查询。
